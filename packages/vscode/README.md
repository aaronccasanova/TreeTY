...
Select a local root, global root, or group before using the title-bar controls. `+` creates a terminal, and `new folder` creates a group at the selected location. Each create prompt confirms its destination. Group hover controls expose create terminal, create group, actions, and delete. Terminal hover controls expose actions and delete; selecting the row creates or reveals its native terminal. The actions gear opens a menu for rename, configure, move, and applicable terminal session actions, so these operations do not depend on discovering the right-click context menu. Right-click remains available with concise action names. The Command Palette groups the same commands under TreeTY. Stopping closes the native terminal but preserves the tree entry, while deleting removes the entry after confirmation.
