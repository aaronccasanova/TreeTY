...
Durable attention from `.treety/state.json` appears as a small `!` file decoration without replacing the lifecycle icon or description. The decoration propagates to ancestor groups. Use the terminal's actions gear to mark it as needing attention before working elsewhere or to clear an existing mark. Opening or revealing the terminal also clears attention.
