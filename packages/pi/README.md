...
Run `/treety-setup` from Pi inside a TreeTY terminal. The command records the current session ID from Pi's session manager, captures Pi's current working directory, configures the terminal to resume it with `pi --session <session-id>` from that directory, and enables lifecycle attention signaling.
