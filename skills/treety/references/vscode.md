...
The setting defaults to `never`. No `projectDir` means no add-folder action. Adding the folder exposes it to Explorer and lets VS Code's native Source Control integration discover repositories there. Use Add Project Directory to VS Code Workspace from a terminal's actions or context menu for an explicit one-time action. TreeTY shows the exact absolute path before changing the workspace.

Group hover controls expose create terminal, create group, actions, and delete. Terminal hover controls expose actions and delete. The actions gear opens a picker for rename, configure, move, and applicable terminal session actions without requiring a right-click. Right-click remains available, and its action labels omit the redundant TreeTY category shown in the global Command Palette.

Use Configure Node from a group or terminal actions menu to edit `cwd`, `projectDir`, environment overrides, freeform metadata, or restart policy. Move opens a searchable, collapsible representation of the tree with every group expanded. Use the top-right controls to expand or collapse the entire tree, or select an individual group to toggle it. Use a destination row's inline controls to insert above, inside a root or group, or below. The editor supports terminals, whole groups, and cross-group moves without generating every possible insertion point as a separate row. Hover a node to inspect its resolved paths and stable ID.
