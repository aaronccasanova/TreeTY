...
Use Configure from a group or terminal actions menu to edit `cwd`, `projectDir`, environment overrides, freeform metadata, or restart policy. Directory prompts preserve direct configuration values. Otherwise, they suggest a live terminal's current directory when VS Code shell integration reports it, then fall back to the resolved node directory. A group can use the active terminal's current directory when that terminal belongs to the group's subtree. Move opens a searchable, collapsible representation of the tree with every group expanded. Use the top-right controls to expand or collapse the entire tree, or select an individual group to toggle it. A destination row orders its inline controls as insert above, insert below, and move inside when applicable, which keeps the inside arrow at the far right. Terminal rows receive an extra indentation step so the hierarchy remains clear. The editor supports terminals, whole groups, and cross-group moves without generating every possible insertion point as a separate row. Hover a node to inspect its resolved paths and stable ID.
