...
Use Configure Node from a group or terminal context menu to edit `cwd`, `projectDir`, environment overrides, freeform metadata, or restart policy. Move opens a searchable, collapsible representation of the tree with every group expanded. Use the top-right controls to expand or collapse the entire tree, or select an individual group to toggle it. Use a destination row's inline controls to insert above, inside a root or group, or below. The editor supports terminals, whole groups, and cross-group moves without generating every possible insertion point as a separate row. Hover a node to inspect its resolved paths and stable ID.
