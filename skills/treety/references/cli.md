...
Install the repository Pi package with `pi install git:github.com/aaronccasanova/TreeTY`, then run `/treety-setup` inside the target TreeTY terminal. The TypeScript extension in `packages/pi` stores the current ID from Pi's session manager through `metadata set-path`, sets `pi --session <session-id>` through `configure`, leaves `restartPolicy` unchanged, and maps `agent_start` and `agent_settled` to generic attention commands. It reconstructs the link during Pi session startup and reload. The extension defines a minimal local Pi API boundary and does not install Pi as a package dependency.
